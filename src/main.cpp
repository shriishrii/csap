...
uint256 hashGenesisBlock("0x00000000048d80e21cdc91f6bb5999c00491490c5f246efc08e9b01829940f32");
...
/** Fees smaller than this (in shrii/satoshi) are considered zero fee (for transaction creation) */
int64 CTransaction::nMinTxFee = 10000;  // Override with -mintxfee
/** Fees smaller than this (in shrii/satoshi) are considered zero fee (for relaying) */
...
const string strMessageMagic = "CSAP Signed Message:\n";
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Go back by what we want to be 14 days worth of blocks (CSAP changed from 14 to 7)
    const CBlockIndex* pindexFirst = pindexLast;
    for (int i = 0; pindexFirst && i < nInterval-1; i++)
        pindexFirst = pindexFirst->pprev;
    assert(pindexFirst);

    // Limit adjustment step (CSAP changed from 4 to 3)
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "The Manila Times, 21/05/2016 - G7 warns of Brexit shock to global economy";
        ...
        block.nVersion = 1;
		block.nTime = 1463816379; // orig 1231006505
		block.nBits = 0x1d00ffff; // orig 0x1d00ffff
		block.nNonce = 2205875483; // orig 2083236893 
		/*
		...
		*/
        if (fTestNet)
        {
			block.nTime = 1463816379; // from 1296688602 to 1463816379 (21/05/2016 00:00:00)
			block.nNonce = 2205875483;
        }
		...
		assert(block.hashMerkleRoot == uint256("0xfe9bd4483356cfc9dfe55a5ef09892a6ad4af8f25cc2f28bb6d8974ffc3867e5"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CSAPMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CSAPMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CSAPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("csap-miner");
    ...
    try { loop {
        ...
        printf("Running CSAPMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
