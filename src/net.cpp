...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
		...
		string strDesc = "CSAP " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
	{"blockchain-tec.net", "csapseed.blockchain-tec.net" },
	{"coinsap.net", "csapseed.coinsap.net" },
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"santapaz.org", "csapseed.santapaz.org"},
	{ NULL, NULL }
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. CSAP is probably already running."), addrBind.ToString().c_str());
        else
