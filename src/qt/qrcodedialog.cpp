...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CSP")
    updateDisplayUnit();
}
...
{
    QString ret = QString("csap:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CSP unit input in lnReqAmount, we generate the URI with CSP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CSP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
